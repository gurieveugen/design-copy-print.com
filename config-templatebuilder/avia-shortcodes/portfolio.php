				...
				$this->elements = array(
					...
					*/
					...
					array(
							...
							"desc" 	=> __("When clicking on a portfolio item you can choose to open the link to the single entry or show a bigger version of the image in a lightbox overlay", 'avia_framework' ),
							"id" 	=> "linking",
							"type" 	=> "select",
							"std" 	=> "",
							"subtype" => array(
...
if ( !class_exists( 'avia_post_grid' ) )
{
	class avia_post_grid
	{
		...
		public function html()
		{
			...
			$output .= "<div class='grid-sort-container isotope {$style_class}-container with-{$contents}-container grid-total-{$total} grid-col-{$columns}' grid-links-{$linking}>";
			...
			foreach ($this->entries->posts as $entry)
			{
				...
				if($linking == 'ajax')
				{
					$preview_templates .= $this->build_preview_template($the_id);
				}
			...
			//apppend preview templates
			if(!empty($preview_templates)) $output .= $preview_templates;
				...
				if(in_array($category->term_id, $current_page_cats))
{
		...
		private function build_preview_template($id)
		{
